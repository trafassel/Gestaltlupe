...
namespace Fractrace.PictureArt
{
    ...
    /// </summary>
    public class DustRenderer : SmallMemoryRenderer
    {
        ...
        public DustRenderer(PictureData pData, ParameterDict parameters)
        ...
        protected override void PreCalculate()
        {
            ...
            _dustscale = (float)_parameters.GetDouble("Renderer.Dustscale");
        ...
        protected Vec3 GetRgb(int x, int y)
        {
            ...
            if (_pictureData.SolidPoints[x, y] != null )
            {
                //   return new Vec3(1, _backColorGreen, _backColorBlue);
            }
        ...
        {
            double d = (_maxPoint.Y - _minPoint.Y) / ((double)pData.Width + pData.Height);
            _heightMap = new float[pData.Width, pData.Height];
            bool onlySolid = _dustlevel > 23;
            for (int i = 0; i < pData.Width; i++)
            {
                for (int j = 0; j < pData.Height; j++)
                {
                    ...
                    FloatPixelInfo solidpInfo = _pictureData.SolidPoints[i, j];
                    if (solidpInfo != null && onlySolid)
                    {
                        _heightMap[i, j] = solidpInfo.Coord.Y;
                        _heightMap[i, j] = _minY;
                    }
                }
            }

            var dustMap = new float[pData.Width, pData.Height];
            for (int i = 0; i < pData.Width; i++)
            {
                for (int j = 0; j < pData.Height; j++)
                {
                    float dustindicator = 0;
                    for (int k = -1; k < 2; k++)
                    {
                        for (int l = -1; l < 2; l++)
                        {
                            int ki = k + i;
                            int lj = l + j;
                            if (ki >= 0 && ki < pData.Width && lj >= 0 && lj < pData.Height)
                            {
                                float diff = Math.Abs(_heightMap[i, j] - _heightMap[ki, lj]);

                                if (diff > _dustscale * d)
                                {
                                    dustindicator++;
                                }
